...
class MusicMan_Track extends MusicMan_Base
{
    ...
    public function create_artist_and_collection($post_id)
    {
        ...
        if ($artist_id && $artist_name) {
            $existing = $this->get_post_by_meta('music_artist', 'artistId', $artist_id);
            if (!$existing) {
                MusicMan_Artist::$skip_auto_import = true;
                $art_post_id = $this->create_or_update_post('music_artist', [
                    'artistId' => $artist_id,
        ...
        if ($coll_id && $coll_name) {
            $existing_coll = $this->get_post_by_meta('music_collection', 'collectionId', $coll_id);
            if (!$existing_coll) {
    ...
    {
        if (is_singular(self::POST_TYPE)) {
            $plugin_template = MUSICMAN_DIR . 'templates/single-music_track.php';
