...
{
    const POST_TYPE = 'musicman_artist';
    ...
    public function import_albums_and_tracks($post_id)
    {
        ...
        $old_track_ids = $this->get_child_track_ids($post_id, 'musicman_artist');
    ...
    public function column_content($column, $post_id)
    {
        switch ($column) {
            ...
            case 'genre':
                ...
                break;
            case 'tracks_count':
                $tracks = $this->get_child_track_ids($post_id, 'musicman_artist');
                echo count($tracks);
                break;
        }
    }
    ...
    {
        return $this->get_child_track_ids($post_id, 'musicman_artist');
    }

    protected function get_child_post_ids($parent_id)
    {
        $collection_ids = get_post_meta($parent_id, '_album_ids', true) ?: [];
        $track_ids = $this->get_child_track_ids($parent_id, 'musicman_artist');
    ...
    public function load_single_template($template)
    {
        if (is_singular(self::POST_TYPE)) {
            $plugin_template = MUSICMAN_DIR . 'templates/single-musicman_artist.php';
